...
const getApprovalChance = (
  ...
  if (emiBurden <= 0.3 && (cibilRange === "780+" || cibilRange === "720-779"))
    return "High";
  return "Medium";
};
...
const inputClass =
  "min-h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-base text-slate-950 outline-none transition focus:border-slate-950 focus:ring-4 focus:ring-slate-100";
...
function ApprovalBadge({ chance }: { chance: ApprovalChance }) {
  ...
  return (
    <span
      className={`inline-flex rounded-full border px-3 py-1 text-sm font-bold ${classes}`}
    >
...
export default function FinancePage() {
  ...
  const emiResult = useMemo(() => {
    ...
    const principalShare =
      totalPayable > 0 ? (principal / totalPayable) * 100 : 0;
    const interestShare =
      totalPayable > 0 ? (totalInterest / totalPayable) * 100 : 0;
    let balance = principal;
    const monthlyRate = rate / 12 / 100;
    const schedule = Array.from({ length: Math.min(months, 8) }).map(
      (_, index) => {
  ...
  const preApprovalResult = useMemo(() => {
    ...
    const estimatedEmi = calculateEmi(
      desiredLoan,
      cibilMidRate[preApproval.cibilRange],
      60
    );
    const approvalChance = getApprovalChance(
      income,
      estimatedEmi + existingEmi,
      preApproval.cibilRange
    );
    const emiBurdenRatio =
      income > 0 ? ((estimatedEmi + existingEmi) / income) * 100 : 0;
    return {
      income,
      existingEmi,
      desiredLoan,
  ...
  const whatsappText = encodeURIComponent(
    `Hi Sangro Finance, my pre-approval estimate is ready.\nName: ${
      preApproval.fullName
    }\nPhone: ${preApproval.phone}\nCity: ${preApproval.city}\nLoan type: ${
      preApproval.loanType
    }\nDesired loan: ${formatMoney(
      preApprovalResult.desiredLoan
    )}\nEligibility estimate: ${formatMoney(
      preApprovalResult.estimatedEligibleAmount
    ...
    try {
      const response = await fetch("/api/service-leads", {
        ...
        body: JSON.stringify({
          ...
          message: `Finance pre-approval request. Eligible EMI: ${formatMoney(
            preApprovalResult.eligibleEmi
          )}. Estimated EMI: ${formatMoney(
            preApprovalResult.estimatedEmi
          )}. EMI burden: ${formatPercent(preApprovalResult.emiBurdenRatio)}.`,
        }),
      });
      ...
      setSubmitError(
        error instanceof Error
          ? error.message
          : "Could not submit finance request."
      );
    } finally {
  ...
  return (
    <main className="min-h-screen bg-white pb-36 pt-16 text-slate-950 md:pb-0 md:pt-0">
      ...
      <nav className="sticky top-0 z-40 hidden border-b border-slate-100 bg-white/90 backdrop-blur-xl md:block">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden items-center gap-6 text-sm font-semibold text-slate-600 md:flex">
            <a href="#emi" className="hover:text-slate-950">
              EMI Calculator
            </a>
            <a href="#pre-approval" className="hover:text-slate-950">
              Pre-Approval
            </a>
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 md:py-12 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-20">
        <div className="flex flex-col justify-center">
          ...
          <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600 sm:mt-6 sm:text-xl">
            Check your EMI, estimate eligibility, and get pre-approved guidance
            before buying your next car.
          </p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <a
              href="#pre-approval"
              className="rounded-2xl bg-slate-950 px-6 py-4 text-center text-base font-black text-white shadow-xl shadow-slate-950/10 transition hover:-translate-y-0.5"
            >
              Check Pre-Approval
            </a>
            <a
              href="#emi"
              className="rounded-2xl border border-slate-300 bg-white px-6 py-4 text-center text-base font-black text-slate-950 transition hover:-translate-y-0.5 hover:border-slate-950"
            >
              Calculate EMI
            </a>
          </div>
          <p className="mt-5 max-w-2xl text-sm text-slate-500">
            Estimated pre-approval only. Final approval depends on bank
            verification.
          </p>
        </div>
        ...
        <div className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-[0_18px_50px_rgba(15,23,42,0.08)] sm:p-5 lg:shadow-[0_30px_100px_rgba(15,23,42,0.10)]">
          <div className="rounded-[1.5rem] bg-slate-950 p-5 text-white">
            ...
            <p className="mt-6 text-4xl font-black tracking-tight">
              {formatMoney(preApprovalResult.estimatedEligibleAmount)}
            </p>
            <p className="mt-2 text-sm text-slate-300">
              Estimated loan eligibility based on FOIR logic
            </p>
            <div className="mt-8 grid grid-cols-2 gap-3">
              <div className="rounded-2xl bg-white/10 p-4">
                <p className="text-xs text-slate-300">Eligible EMI</p>
                <p className="mt-2 text-xl font-black">
                  {formatMoney(preApprovalResult.eligibleEmi)}
                </p>
              </div>
              <div className="rounded-2xl bg-white/10 p-4">
                <p className="text-xs text-slate-300">Interest range</p>
                <p className="mt-2 text-xl font-black">
                  {preApprovalResult.interestRange}
                </p>
              </div>
            </div>
          </div>
          <div className="mt-5 space-y-4">
            {[
              ...
              [
                ...
                Math.min(
                  (preApprovalResult.eligibleEmi /
                    Math.max(preApprovalResult.income || 1, 1)) *
                    100,
                  100
                ),
              ],
              [
                ...
                Math.min(
                  (parseMoney(emiForm.downPayment) /
                    Math.max(parseMoney(emiForm.loanAmount), 1)) *
                    100,
                  100
                ),
              ],
      ...
      <section className="mx-auto grid max-w-7xl gap-4 px-4 pb-6 sm:grid-cols-2 sm:px-6 lg:grid-cols-4 lg:px-8">
        <StatCard
          label="Loan eligibility"
          value={formatMoney(preApprovalResult.estimatedEligibleAmount)}
          helper="FOIR-based estimate"
        />
        <StatCard
          label="EMI burden"
          value={formatPercent(preApprovalResult.emiBurdenRatio)}
          helper="Good below 40%"
        />
        <StatCard
          label="Suggested loan"
          value={`${formatMoney(preApprovalResult.rangeLow)} - ${formatMoney(
            preApprovalResult.rangeHigh
          )}`}
          helper="±20% range"
        />
        <StatCard
          label="Debt health"
          value={
            preApprovalResult.approvalChance === "High"
              ? "Good"
              : preApprovalResult.approvalChance === "Medium"
              ? "Watch"
              : "Risky"
          }
          helper="Based on income + CIBIL"
        />
      </section>

      <section
        id="emi"
        className="mx-auto grid max-w-7xl gap-6 px-4 py-8 sm:px-6 md:py-12 lg:grid-cols-[0.9fr_1.1fr] lg:px-8"
      >
        <div>
          ...
          <h2 className="mt-3 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">
            Plan the monthly payment before you buy.
          </h2>
          <p className="mt-4 text-base leading-7 text-slate-600 sm:text-lg">
            Use the standard EMI formula with loan amount, down payment,
            processing fee, interest rate and tenure.
          </p>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            <StatCard
              label="Monthly EMI"
              value={formatMoney(emiResult.monthlyEmi)}
            />
            <StatCard
              label="Total interest"
              value={formatMoney(emiResult.totalInterest)}
            />
            <StatCard
              label="Total payable"
              value={formatMoney(emiResult.totalPayable)}
            />
            <StatCard
              label="Financed principal"
              value={formatMoney(emiResult.principal)}
            />
          </div>
        </div>
        ...
        <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-[0_16px_50px_rgba(15,23,42,0.06)] sm:p-6">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Loan amount">
              <input
                ...
                onChange={(event) =>
                  setEmiForm({ ...emiForm, loanAmount: event.target.value })
                }
              />
            </Field>
            <Field label="Down payment">
              <input
                ...
                onChange={(event) =>
                  setEmiForm({ ...emiForm, downPayment: event.target.value })
                }
              />
            </Field>
            <Field label="Interest rate (%)">
              <input
                ...
                onChange={(event) =>
                  setEmiForm({ ...emiForm, interestRate: event.target.value })
                }
              />
            </Field>
            <Field label="Tenure in months">
              <input
                ...
                onChange={(event) =>
                  setEmiForm({ ...emiForm, tenureMonths: event.target.value })
                }
              />
            </Field>
            <Field label="Processing fee optional">
              <input
                ...
                onChange={(event) =>
                  setEmiForm({ ...emiForm, processingFee: event.target.value })
                }
              />
            </Field>
          </div>
          ...
          <div className="mt-8 rounded-3xl border border-slate-200 p-5">
            <div className="flex h-5 overflow-hidden rounded-full bg-slate-100">
              <div
                ...
                style={{ width: `${emiResult.principalShare}%` }}
              />
              <div
                className="bg-emerald-500"
                style={{ width: `${emiResult.interestShare}%` }}
              />
            </div>
            <div className="mt-3 flex flex-wrap gap-4 text-sm text-slate-600">
              <span className="inline-flex items-center gap-2">
                <span className="h-3 w-3 rounded-full bg-slate-950" /> Principal
              </span>
              <span className="inline-flex items-center gap-2">
          ...
          <div className="mt-6 hidden overflow-hidden rounded-3xl border border-slate-200 sm:block">
          ...
          <div className="mt-6 grid gap-3 sm:hidden">
            {emiResult.schedule.slice(0, 4).map((row) => (
              <div
                key={row.month}
              ...
              >
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                  <span>Interest {formatMoney(row.interest)}</span>
      ...
      <section
        id="pre-approval"
        className="mx-auto grid max-w-7xl gap-6 px-4 py-8 sm:px-6 md:py-12 lg:grid-cols-[1fr_0.9fr] lg:px-8"
      >
        <form
          onSubmit={submitPreApproval}
        ...
        >
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Phone">
              <input
                className={inputClass}
                value={preApproval.phone}
                onChange={(event) =>
                  setPreApproval({ ...preApproval, phone: event.target.value })
                }
                required
              />
            </Field>
            <Field label="City">
              <input
                ...
                onChange={(event) =>
                  setPreApproval({ ...preApproval, city: event.target.value })
                }
              />
            </Field>
            <Field label="Monthly income">
              <input
                className={inputClass}
                value={preApproval.monthlyIncome}
                onChange={(event) =>
                  setPreApproval({
                    ...preApproval,
            ...
            <Field label="Existing EMI">
              <input
                className={inputClass}
                value={preApproval.existingEmi}
                onChange={(event) =>
                  setPreApproval({
                    ...preApproval,
                    existingEmi: event.target.value,
                  })
                }
              />
            </Field>
            <Field label="Employment type">
              <select
                className={inputClass}
                value={preApproval.employmentType}
                onChange={(event) =>
                  setPreApproval({
                    ...preApproval,
            ...
            <Field label="CIBIL range">
              <select
                className={inputClass}
                value={preApproval.cibilRange}
                onChange={(event) =>
                  setPreApproval({
                    ...preApproval,
                    cibilRange: event.target.value as CibilRange,
                  })
                }
              >
                <option>780+</option>
            ...
            <Field label="Loan type">
              <select
                className={inputClass}
                value={preApproval.loanType}
                onChange={(event) =>
                  setPreApproval({
                    ...preApproval,
                    loanType: event.target.value as LoanType,
                  })
                }
              >
            ...
            <Field label="Desired loan amount">
              <input
                className={inputClass}
                value={preApproval.desiredLoanAmount}
                onChange={(event) =>
                  setPreApproval({
                    ...preApproval,
                    desiredLoanAmount: event.target.value,
                  })
                }
              />
            </Field>
          ...
          {submitted ? (
            <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800">
              Your pre-approval estimate is ready. Sangro Finance will call you
              and help process this through our banking partners.
            </div>
          ) : null}
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <button
              ...
              className="rounded-2xl bg-slate-950 px-6 py-4 font-black text-white disabled:cursor-not-allowed disabled:opacity-60"
              type="submit"
            >
            ...
            <a
              ...
              className="rounded-2xl border border-slate-300 px-6 py-4 text-center font-black text-slate-950 hover:border-slate-950"
            >
          ...
          <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_16px_50px_rgba(15,23,42,0.06)]">
            ...
            <div className="mt-6 grid gap-4">
              <StatCard
                ...
                value={formatMoney(preApprovalResult.estimatedEligibleAmount)}
                helper={`${formatMoney(
                  preApprovalResult.rangeLow
                )} - ${formatMoney(preApprovalResult.rangeHigh)} range`}
              />
              <StatCard
                label="Suggested EMI limit"
                value={formatMoney(preApprovalResult.eligibleEmi)}
                helper="Monthly income × 40% minus existing EMI"
              />
              <StatCard
                label="Estimated interest range"
                value={preApprovalResult.interestRange}
                helper="Based on selected CIBIL range"
              />
            </div>
          </div>
          <div className="rounded-[2rem] border border-slate-200 bg-slate-950 p-6 text-white shadow-[0_16px_50px_rgba(15,23,42,0.10)]">
            ...
            <div className="mt-5 grid gap-3">
              {documents.map((document) => (
                <div
                  key={document}
                  className="flex items-center gap-3 rounded-2xl bg-white/10 px-4 py-3 text-sm font-semibold text-slate-100"
                >
      ...
      >
        <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-[0_16px_50px_rgba(15,23,42,0.06)] sm:p-8">
          <p className="text-sm font-black uppercase tracking-[0.22em] text-emerald-700">
            Bank partner process
          </p>
          <h2 className="mt-3 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">
            From estimate to bank review.
          </h2>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-8 sm:px-6 md:pb-14 lg:px-8">
        <div className="rounded-3xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-900">
          <strong>Disclaimer:</strong> Sangro Finance does not guarantee loan
          approval. Final approval, interest rate, and disbursal are subject to
          bank/NBFC checks, documents, credit score, and policies.
        </div>
      </section>
    </main>
  );
