...
export async function POST(req: Request) {
  const redirectWithError = (
    ...
    NextResponse.redirect(
      new URL(
        `/admin/listings/new?error=${encodeURIComponent(error)}&message=${encodeURIComponent(message)}`,
        req.url
      )
    );
  ...
  try {
    ...
    if (!make || !model) {
      if (wantsJson) {
        return NextResponse.json(
          { ok: false, error: "missing_fields", message: "Please fill required fields (make and model)." },
          { status: 400 }
        );
      }
      return redirectWithError("missing_fields", "Please fill required fields (make and model).");
    }
    ...
    if (isPrivateSeller && (!sellerName || !sellerPhone)) {
      ...
      return redirectWithError(body.error, body.message);
    }
    ...
    const description = withDealerSubmittedPrice(
      buildListingExperienceDescription(
        {
    ...
    console.error("Admin listing route crashed", error);
    const message =
      error instanceof Error && error.message
        ? error.message
        : "Could not create listing. Please try again.";
    if (req.headers.get("x-admin-form") === "1") {
      return NextResponse.json(
        { ok: false, error: "create_failed", message },
        { status: 500 }
      );
    }
    return redirectWithError("create_failed", message);
  }
}
